	...
	if ($('.plupload').length > 0) {
		...
		$(".plupload").each(function () {
			...
			$el.pluploadQueue({
				...
				flash_swf_url      : window.assetsUrl+'js/plupload/Moxie.swf',
				silverlight_xap_url: window.assetsUrl+'js/plupload/Moxie.xap'
			});
		...
		$('.dataTable').each(function () {
			...
			if ($el.hasClass('dataTable-tools')) {
				...
				dataTable_options.tableTools = {
					"sSwfPath": window.assetsUrl+"js/plugins/datatables/extensions/copy_csv_xls_pdf.swf"
				};
			}
